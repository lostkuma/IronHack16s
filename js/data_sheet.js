...
// 	1. freshness feature affected by current weather training result(node) from python
//		* every decision tree node gotten from training weather data in python
//		* ^ are stored in an array that can be directly used from predicting freshness based on current weather data
//
// 	2. freshness feature affected by surface water data (daily mean in ft^3/s from 1923-10-1 to 2015-9-30)
// 	3. freshness feature affected by in season vegetables cout (in month --12)
// 	4. freshness feature affected by local farmers' market seasonal open duration data (in month --12)
//	5. freshness feature affected by storm data (average monthly count from 2011-2015)
//	6. freshness feature affected by hail data (average monthly count from 2011-2015)
//
//	^ everything is scaled to a 1 to 10 scale	
//
// the above five features will be combined to calculate a current freshness level with assigned weights
